    ...
    <div id="offCanvasWrapper" class="mui-off-canvas-wrap mui-draggable mui-slide-in">
      ...
      <div class="mui-inner-wrap">
        <header class="mui-bar mui-bar-nav" style="background-color: #007AFF;">
          ...
          <h1 class="mui-title" style="color: white;">已删除</h1>
        </header>
          ...
          <div class="mui-scroll">
            ...
            <ul class="mui-table-view">
                ...
                <div class="mui-slider-right mui-disabled">
                	<a @tap="restore(item,i)" class="mui-btn mui-btn-yellow mui-icon mui-icon-undo"></a>
                	<a @tap="deleteMail(item,i)" class="mui-btn mui-btn-red mui-icon mui-icon-trash"></a>
                </div>
                <div @tap="toDetail(item,i)" class="mui-slider-handle">
                  <img v-if="item.senderHeadImg == null || item.senderHeadImg == '' || item.senderHeadImg.length == 0" class="round_icon" src="../assets/images/headImg.jpeg">
                  <img v-if="item.senderHeadImg != null && item.senderHeadImg != '' && item.senderHeadImg.length != 0" class="round_icon" :src="item.senderHeadImg">
                  <div class="mui-media-body">
                    ...
                    <div style="margin-left: 10px;">
                      ...
                      <div style="width: 10%;float: right;">
                        <span class="mui-icon" v-bind:class="starStatus[i]" style="font-size: 19px;float: right;padding-right: 0px;"></span>
                      </div>
                    </div>
                  </div>
                  <div class="meta-info">
...
<script>
  export default {
    ...
    methods: {
      ...
      getDeletedMails: function() {
        console.log('come in get deleted mails')
      ...
      restore: function(item,i) {
        var that = this
        var curChild = this.$refs.parentLi[i]; // 通过ref获取当前点击的列表
        this.$mui.confirm(' ', '确认恢复该邮件？', ['确认', '取消'], function(e) {
          if (e.index == 0) {
            that.$http({
              method: 'put',
              url: '/mail/updateMail',
      ...
      deleteMail: function(item,i) {
        var that = this
        var curChild = this.$refs.parentLi[i]; // 通过ref获取当前点击的列表
        this.$mui.confirm(' ', '确认永久删除该邮件？', ['确认', '取消'], function(e) {
          if (e.index == 0) {
            that.$http({
              method: 'put',
              url: '/mail/updateMail',
