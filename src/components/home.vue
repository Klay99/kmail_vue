    ...
    <div id="offCanvasWrapper" class="mui-off-canvas-wrap mui-draggable mui-slide-in">
      ...
      <div class="mui-inner-wrap">
        <header class="mui-bar mui-bar-nav" style="background-color: #007AFF;">
          ...
          <h1 class="mui-title" style="color: white;">收件箱</h1>
        </header>
        <div id="offCanvasContentScroll" class="mui-content mui-scroll-wrapper">
          <!-- 固定导航，写邮件 -->
          <div class="addMail" v-if="mailList != null || mailList != '' || mailList.length != 0">
            <span @tap="toBuildMail" class="mui-icon mui-icon-plus-filled" style="font-size: 48px;float: right;padding-right: 20px;"></span>
          </div>
          <!-- 邮件列表 -->
          <div class="mui-scroll">
  ...
  export default {
    ...
    methods: {
      ...
      deleteMail: function(item,i) {
        ...
        this.$mui.confirm(' ', '确认删除该邮件？', ['确认', '取消'], function(e) {
          if (e.index == 0) {
            that.$http({
              method: 'put',
              url: '/mail/updateMail',
    ...
    filters: { // 过滤器
      dateTrans: function(val) {
        ...
        if(diff < 60000) {
          ...
          return date.getDate() + '日 ' + date.getHours() + ':' + date.getMinutes() + '  >'
